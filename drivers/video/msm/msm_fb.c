			...
			unsigned long arg)
{
	...
	case MSMFB_REG_WRITE:
		if (copy_from_user(&reg_access, (void __user *)arg,
							sizeof(reg_access)))
			return -EFAULT;

		reg_access_buf = kmalloc(reg_access.buffer_size + 1,
								GFP_KERNEL);
		if (!reg_access_buf)
			return -ENOMEM;
		...
		ret = mfd->reg_write(mfd, reg_access.buffer_size + 1,
	...
	case MSMFB_REG_READ:
		...
		ret = mfd->reg_read(mfd, reg_access.address,
